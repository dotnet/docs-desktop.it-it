...
## <a name="example"></a>Esempio  
 L'esempio di codice seguente illustra come usare un componente <xref:System.ComponentModel.BackgroundWorker> per caricare un file XML da un URL. Quando l'utente fa clic sul pulsante **download** , il <xref:System.Windows.Forms.Control.Click> gestore eventi chiama il <xref:System.ComponentModel.BackgroundWorker.RunWorkerAsync%2A> metodo di un <xref:System.ComponentModel.BackgroundWorker> componente per avviare l'operazione di download. Il pulsante è disattivato per la durata del download e quindi abilitato una volta che il download è completato. Un oggetto <xref:System.Windows.Forms.MessageBox> visualizza i contenuti del file.  
