...
6. Individuare il `InitializeComponent` metodo e rimuovere la riga che assegna la <xref:System.Windows.Forms.ContainerControl.AutoScaleMode%2A> Proprietà. Questa proprietà non esiste nel <xref:System.Windows.Forms.Button> controllo.

7. Per salvare il progetto, scegliere **Salva tutto** dal menu **File**.

    > [!NOTE]
    > Non è più disponibile alcuna finestra di progettazione. Poiché il <xref:System.Windows.Forms.Button> controllo esegue un disegno personalizzato, non è possibile modificarne l'aspetto nella finestra di progettazione. La relativa rappresentazione visiva sarà esattamente identica a quella della classe da cui eredita (ovvero), a <xref:System.Windows.Forms.Button> meno che non venga modificata nel codice. È comunque possibile aggiungere nell'area di progettazione i componenti che non dispongono di elementi dell'interfaccia utente.
...
Nel menu **Compila** scegliere **Compila soluzione**. La compilazione dovrebbe essere completata senza errori del compilatore o avvisi.
