...
Il <xref:System.ComponentModel.BackgroundWorker> componente sostituisce e aggiunge funzionalità allo <xref:System.Threading> spazio dei nomi. Tuttavia, lo <xref:System.Threading> spazio dei nomi viene mantenuto sia per la compatibilità con le versioni precedenti che per un uso futuro, se si sceglie. Per ulteriori informazioni, vedere [Cenni preliminari sul componente BackgroundWorker](backgroundworker-component-overview.md).

 Windows Forms usa il modello STA (Single-Threaded Apartment) perché Windows Forms si basa su finestre Win32 native che sono intrinsecamente sottoposto a threading Apartment. Il modello STA implica che una finestra può essere creata in qualsiasi thread, ma non può cambiare thread una volta creati e tutte le chiamate di funzione devono essere eseguite nel thread di creazione. All'esterno di Windows Forms, le classi della .NET Framework utilizzano il modello di threading libero. Per informazioni sul threading nella .NET Framework, vedere [Threading](/dotnet/standard/threading/index).

 Il modello STA richiede che tutti i metodi di un controllo che devono essere chiamati dall'esterno del thread di creazione del controllo debbano essere sottoposti a marshalling in (eseguito su) il thread di creazione del controllo. La classe base <xref:System.Windows.Forms.Control> fornisce diversi metodi ( <xref:System.Windows.Forms.Control.Invoke%2A> , <xref:System.Windows.Forms.Control.BeginInvoke%2A> e <xref:System.Windows.Forms.Control.EndInvoke%2A> ) a questo scopo. <xref:System.Windows.Forms.Control.Invoke%2A> esegue chiamate al metodo sincrono; <xref:System.Windows.Forms.Control.BeginInvoke%2A> esegue chiamate asincrone al metodo.

 Se si usa il multithreading nel controllo per le attività a elevato utilizzo di risorse, l'interfaccia utente può rimanere attiva mentre un calcolo a elevato utilizzo di risorse viene eseguito in un thread in background.

 Nell'esempio seguente ( `DirectorySearcher` ) viene illustrato un controllo Windows Forms multithreading che utilizza un thread in background per eseguire una ricerca ricorsiva in una directory per individuare i file corrispondenti a una stringa di ricerca specificata e quindi popolare una casella di riepilogo con i risultati della ricerca. I concetti chiave illustrati nell'esempio sono i seguenti:

- `DirectorySearcher` avvia un nuovo thread per eseguire la ricerca. Il thread esegue il `ThreadProcedure` metodo che a sua volta chiama il metodo helper `RecurseDirectory` per eseguire la ricerca effettiva e per popolare la casella di riepilogo. Tuttavia, il popolamento della casella di riepilogo richiede una chiamata cross-thread, come illustrato nei due elementi puntati successivi.

- `DirectorySearcher` definisce il `AddFiles` metodo per aggiungere file a una casella di riepilogo. Tuttavia, `RecurseDirectory` non può richiamare direttamente `AddFiles` perché `AddFiles` può essere eseguito solo nel thread sta che ha creato `DirectorySearcher` .

- L'unico modo in `RecurseDirectory` `AddFiles` cui è possibile chiamare è tramite una chiamata cross-thread, ovvero chiamando <xref:System.Windows.Forms.Control.Invoke%2A> o <xref:System.Windows.Forms.Control.BeginInvoke%2A> per effettuare `AddFiles` il marshalling al thread di creazione di `DirectorySearcher` . `RecurseDirectory` USA <xref:System.Windows.Forms.Control.BeginInvoke%2A> in modo che la chiamata possa essere eseguita in modo asincrono.

- Il marshalling di un metodo richiede l'equivalente di un puntatore a funzione o di un callback. Questa operazione viene eseguita utilizzando delegati nell'.NET Framework. <xref:System.Windows.Forms.Control.BeginInvoke%2A> accetta un delegato come argomento. `DirectorySearcher` definisce pertanto un delegato ( `FileListDelegate` ), viene associato `AddFiles` a un'istanza di `FileListDelegate` nel relativo costruttore e passa questa istanza del delegato a <xref:System.Windows.Forms.Control.BeginInvoke%2A> . `DirectorySearcher` definisce anche un delegato dell'evento di cui viene effettuato il marshalling quando la ricerca viene completata.
...
## <a name="using-the-multithreaded-control-on-a-form"></a>Uso del controllo multithread in un form

 Nell'esempio seguente viene illustrato il modo `DirectorySearcher` in cui il controllo multithread può essere utilizzato in un form.
