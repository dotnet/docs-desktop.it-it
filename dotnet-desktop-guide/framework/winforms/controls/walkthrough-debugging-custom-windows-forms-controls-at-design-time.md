...
1. Aprire il file di origine **DebugControl** nell' **editor di codice** e inserire un punto di interruzione nella `Set` funzione di accesso della `DemoString` Proprietà.
...
- È possibile utilizzare la <xref:System.ComponentModel.Component.DesignMode%2A> proprietà della <xref:System.ComponentModel.Component> classe per scrivere codice che verrà eseguito solo in fase di progettazione. Per informazioni dettagliate, vedere <xref:System.ComponentModel.Component.DesignMode%2A>.
