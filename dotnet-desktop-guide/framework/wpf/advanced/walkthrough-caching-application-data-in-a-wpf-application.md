...
#### <a name="to-change-the-target-net-framework-in-a-visual-c-project"></a>Per modificare la .NET Framework di destinazione in un progetto Visual C#
...
## <a name="adding-a-button-to-the-wpf-window"></a>Aggiunta di un pulsante alla finestra WPF
 Successivamente, si aggiungerà un controllo Button e si creerà un gestore eventi per l'evento del pulsante `Click` . Successivamente, quando si fa clic sul pulsante viene aggiunto il codice, il contenuto del file di testo viene memorizzato nella cache e visualizzato.
...
2. Dalla **casella degli strumenti**, sotto **controlli WPF comuni**, trascinare un `Button` controllo nella `MainWindow` finestra.
...
- Consente di specificare che la cache utilizza un <xref:System.Runtime.Caching.HostFileChangeMonitor> oggetto per monitorare le modifiche nel file di testo.
...
2. Aggiungere le seguenti `Imports` istruzioni (Visual Basic) o (C#) all'inizio del file (prima della dichiarazione di classe) `using` :
...
6. Nel `if/then` blocco aggiungere il codice seguente per creare un nuovo <xref:System.Runtime.Caching.CacheItemPolicy> oggetto che specifichi che la voce della cache scade dopo 10 secondi.
...
7. All'interno del `if/then` blocco e dopo il codice aggiunto nel passaggio precedente, aggiungere il codice seguente per creare una raccolta per i percorsi di file che si desidera monitorare e per aggiungere il percorso del file di testo alla raccolta:
...
1. Premere CTRL+F5 per eseguire l'applicazione.
     ...
     `MainWindow`Viene visualizzata la finestra.
...
- [Memorizzazione nella cache in applicazioni .NET Framework](/dotnet/framework/performance/caching-in-net-framework-applications)
